...
func TestResolveMode_Table(t *testing.T) {
	cases := []struct {
		...
		{"auto+missing_downgrades", "auto", false, "zstd not on $PATH", "off", nil},
		{"off_skips_probe_even_when_missing", "off", false, "zstd not on $PATH", "off", nil},
		{"required+ok", "required", true, "", "auto", nil},
		{"required+missing_hardfails", "required", false, "zstd not on $PATH", "", zstdpatch.ErrZstdBinaryMissing},
	}
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			probe := func(context.Context) (bool, string) { return tc.probeOK, tc.reason }
			eff, err := resolveMode(context.Background(), tc.input, probe)
			if tc.wantErrIs != nil {
				require.Error(t, err)
...
func TestResolveMode_UnknownValueRejected(t *testing.T) {
	...
	_, err := resolveMode(context.Background(), "aggressive", probe)
	require.Error(t, err)
