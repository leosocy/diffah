...
14. **Per-image baseline-completeness preflight (hardening PR4).** Before `importEachImage` constructs its admission pool, every image in `applyList` is checked against its resolved baseline source for the presence and digest integrity of all baseline-only layer digests declared in its target manifest. Cross-image masking through the digest-keyed shared spool is impossible at any worker count, including corrupt per-image baseline bytes for a digest another image can provide. Failures surface as `*ErrMissingBaselineReuseLayer` or `*diff.ErrBaselineBlobDigestMismatch` in the apply report under `PreflightBaselineMissing` status.
