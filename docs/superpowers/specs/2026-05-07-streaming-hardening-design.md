...
3. For each baseline-only digest, check the resolved baseline source for THAT image. The check uses `types.ImageSource.GetBlob`, drains the returned stream, and verifies the bytes hash to the requested digest; this mirrors `fetchVerifiedBaselineBlob`'s integrity contract without writing into the shared spool.
4. Missing digests surface as `*ErrMissingBaselineReuseLayer{ImageName, LayerDigest}`. Digest mismatches surface as `*diff.ErrBaselineBlobDigestMismatch{ImageName, Digest, Got}`. In both cases the image is moved from `applyList` to `skippedByPreflight` with a new `PreflightStatus` value `PreflightBaselineMissing`.

The check uses the same `resolvedByName` map `Import` already builds, so no new resolution work. The cost is `O(images × baseline-only-layer-bytes)` pre-apply verification I/O against baseline transports. This is intentionally more expensive than a presence probe because a digest-keyed shared spool can otherwise mask a corrupt per-image baseline when a sibling image has already populated the same digest.

**Why drain-and-hash, not HEAD.** `types.ImageSource` does not expose HEAD directly; `GetBlob` is the portable shape across `oci:`, `docker://`, `dir:`, and `containers-storage:`. A 1-byte fetch proves presence but not per-image integrity, so it is insufficient for the cross-image corruption-masking variant of P0 #2. If a future operator profiles preflight as too slow, an opt-out flag (`--no-baseline-preflight`) can be added; this initiative ships correctness-first preflight.
